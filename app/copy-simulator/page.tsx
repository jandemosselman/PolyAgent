...
export default function CopySimulatorPage() {
  ...
  const [bestStrategies, setBestStrategies] = useState<Array<{
    ...
    sharpeRatio: number
    maxDrawdown: number
    avgTradesToProfit10: number
    avgTradesToProfit25: number
    avgTradesToProfit50: number
  ...
  */
  ...
  const pushToBot = async () => {
    setSyncingWithBot(true)
    try {
      const railwayUrl = process.env.NEXT_PUBLIC_RAILWAY_BOT_URL || localStorage.getItem('railwayBotUrl')
      
      if (!railwayUrl) {
        throw new Error('RAILWAY_BOT_URL not configured')
      }
      
      console.log('🚀 Pushing local configurations to Railway bot...')
      
      // Send ONLY the active local runs to the bot
      const botConfigs = copyTrades
        .filter(ct => !ct.isArchived) // Don't sync archived runs
        .map(ct => ({
          id: ct.id,
          name: ct.name,
          traderAddress: ct.traderAddress,
      ...
      const result = await response.json()
      console.log('✅ Bot updated successfully:', result)
      
      setNotification({
        message: `✅ Pushed ${botConfigs.length} run(s) to bot. Deleted runs removed from bot!`,
        type: 'success'
      })
      ...
      fetch('/api/telegram', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          message: `🚀 *Pushed to bot*\n\n${botConfigs.length} configuration(s) synced to Railway`
        })
      }).catch(console.error)
      
    } catch (error: any) {
      console.error('❌ Error pushing to bot:', error)
...
📊 Current Budget: $${copyTrade.currentBudget.toFixed(2)}
  ...
  const deleteConfiguration = async (group: any) => {
    const configName = configNames[group.configKey] || `Configuration #${strategyGroups.indexOf(group) + 1}`
    ...
    try {
      ...
      // Auto-sync deletion to Railway bot
      const railwayUrl = process.env.NEXT_PUBLIC_RAILWAY_BOT_URL || localStorage.getItem('railwayBotUrl')
      if (railwayUrl) {
        try {
  ...
  const findBestStrategy = async (group: any) => {
    ...
    // CRITICAL: Determine actual price range from the data
    const tradePrices = allTrades.map(t => t.price).filter(p => p > 0)
    const minDataPrice = Math.min(...tradePrices)
    const maxDataPrice = Math.max(...tradePrices)
    console.log(`📊 Available data price range: ${minDataPrice.toFixed(2)} - ${maxDataPrice.toFixed(2)}`)
    
    // SMART: Generate price ranges ONLY within the available data
    const priceRanges: Array<{ min: number, max: number }> = []
    
    // Round to reasonable boundaries
    const dataMin = Math.floor(minDataPrice * 20) / 20  // Round to 0.05
    const dataMax = Math.ceil(maxDataPrice * 20) / 20
    
    // Generate ranges within actual data bounds
    const rangeSize = dataMax - dataMin
    
    if (rangeSize >= 0.20) {
      // Large dataset - test multiple sub-ranges
      for (let min = dataMin; min <= dataMax - 0.10; min += 0.03) {
        for (let width = 0.10; width <= Math.min(0.25, dataMax - min); width += 0.05) {
          const max = Math.min(min + width, dataMax)
          priceRanges.push({ 
    ...
    } else if (rangeSize >= 0.10) {
      // Medium dataset - test a few ranges
      priceRanges.push({ min: dataMin, max: dataMax }) // Full range
      const mid = (dataMin + dataMax) / 2
      priceRanges.push({ min: dataMin, max: mid })
      priceRanges.push({ min: mid, max: dataMax })
    } else {
      // Small range - just use the full available range
      priceRanges.push({ min: dataMin, max: dataMax })
    }
    
    // Remove duplicates and sort
    const uniqueRanges = priceRanges.filter((range, index, self) => 
      index === self.findIndex(r => r.min === range.min && r.max === range.max)
    ).sort((a, b) => a.min - b.min)
    
    console.log(`✅ Testing ${uniqueRanges.length} price ranges (all within available data)`)
    uniqueRanges.slice(0, 5).forEach(r => console.log(`  - ${r.min.toFixed(2)} to ${r.max.toFixed(2)}`))
    if (uniqueRanges.length > 5) console.log(`  ... and ${uniqueRanges.length - 5} more`)
    ...
    if (mcBettingMode === 'fixed') {
      ...
      totalCombinations = uniqueRanges.length * minTriggers.length * betValues.length
    ...
    for (const priceRange of uniqueRanges) {
      for (const minTrigger of minTriggers) {
        for (const betValue of betValues) {
          ...
          setStrategyFinderProgress(`Testing ${combinationIndex}/${totalCombinations}: ${mcBettingMode.toUpperCase()} ${modeLabel}, Price ${priceRange.min.toFixed(2)}-${priceRange.max.toFixed(2)}, Trigger $${minTrigger}`)
          ...
          for (let i = 0; i < numSims; i++) {
            ...
            let tradesToProfit10 = -1
            let tradesToProfit25 = -1
            let tradesToProfit50 = -1
            let currentLosses = 0
            let maxLosses = 0
            ...
            for (let j = startIndex; j < filteredTrades.length; j++) {
              ...
              if (trade.status === 'won') {
                ...
                budget += payout
                wins++
                currentLosses = 0  // Reset losing streak
              } else {
                currentLosses++
              ...
              const currentProfit = ((budget - mcBudget) / mcBudget) * 100
              if (tradesToProfit10 === -1 && currentProfit >= 10) tradesToProfit10 = tradesCopied
              if (tradesToProfit25 === -1 && currentProfit >= 25) tradesToProfit25 = tradesCopied
              if (tradesToProfit50 === -1 && currentProfit >= 50) tradesToProfit50 = tradesCopied
            }
          ...
          // ENHANCED: Profit milestone averages (only for sims that reached milestone)
          const simsReached10 = sims.filter(s => !s.bankrupt && s.tradesToProfit10 > 0)
          const avgTradesToProfit10 = simsReached10.length > 0 
            ? simsReached10.reduce((sum, s) => sum + s.tradesToProfit10, 0) / simsReached10.length 
            : -1
          
          const simsReached25 = sims.filter(s => !s.bankrupt && s.tradesToProfit25 > 0)
          const avgTradesToProfit25 = simsReached25.length > 0 
            ? simsReached25.reduce((sum, s) => sum + s.tradesToProfit25, 0) / simsReached25.length 
            : -1
          
          const simsReached50 = sims.filter(s => !s.bankrupt && s.tradesToProfit50 > 0)
          const avgTradesToProfit50 = simsReached50.length > 0 
            ? simsReached50.reduce((sum, s) => sum + s.tradesToProfit50, 0) / simsReached50.length 
            : -1
          
          // ENHANCED: Average ROI per trade
          const avgRoiPerTrade = avgTrades > 0 ? (avgPnl / mcBudget / avgTrades) * 100 : 0
          
          // ENHANCED: Worst losing streak
          const worstStreak = sims.reduce((max, s) => Math.max(max, s.maxLosses), 0)
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              title="Pull data FROM Railway bot"
            >
              {syncingWithBot ? '🔄 Syncing...' : '⬇️ Pull from Bot'}
            </button>
            <button
              onClick={pushToBot}
              disabled={syncingWithBot}
              className="px-4 py-2 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 hover:border-emerald-500/50 text-emerald-400 font-medium rounded-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              title="Push local configs TO Railway bot (overwrites bot, removes deleted runs)"
            >
              {syncingWithBot ? '🔄 Pushing...' : '⬆️ Push to Bot'}
            </button>
            <button
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl max-w-3xl w-full p-6 max-h-[90vh] overflow-y-auto">
              ...
              <div className="mb-6 pb-6 border-b border-slate-800/50">
                ...
                {bestStrategies && bestStrategies.length > 0 && (
                  <div className="mt-4 bg-gradient-to-br from-emerald-900/30 to-green-900/20 border-2 border-emerald-500/50 rounded-xl p-6">
                    <h4 className="text-lg font-semibold text-emerald-300 mb-2">🛡️ Survival-Focused Strategies for ${mcBudget} Budget</h4>
                    <p className="text-sm text-slate-400 mb-4">Prioritizing: Zero bankruptcy &gt; Consistency &gt; Steady growth</p>
                    
                    <div className="space-y-4">
                      {bestStrategies.map((strategy, index) => (
                        <div key={index} className={`bg-slate-900/50 rounded-lg p-4 border-2 ${
                          index === 0 ? 'border-emerald-500/70' : 'border-slate-700/30'
                        }`}>
                          <div className="flex items-start justify-between mb-3">
                            <div>
                              <span className="text-lg font-bold text-slate-200">
                                ...
                                {index === 1 && '🥈 '}
                                {index === 2 && '🥉 '}
                                #{index + 1} {strategy.bankruptcyRate === 0 && '💎 ZERO BANKRUPTCY'}
                              </span>
                              <div className="mt-1 text-sm text-slate-400">
                                ...
                                )} | Price {strategy.minPrice.toFixed(2)}-{strategy.maxPrice.toFixed(2)} | Trigger ${strategy.minTrigger}
                              </div>
                              <div className="mt-1 text-xs text-slate-500">
                                📊 Based on <span className="text-cyan-400 font-semibold">{Math.round(strategy.totalTrades)} avg trades</span> per simulation (100 sims)
                              </div>
                            </div>
                            <div className="flex gap-2">
                              <button
                                ...
                                className="px-4 py-2 bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-400 font-medium text-sm rounded-lg transition-all whitespace-nowrap"
                              >
                                📋 Use This
                              </button>
                              <button
                                onClick={async () => {
                                  ...
                                  const runName = prompt('Enter run name:', `${group.runs[0]?.name || 'Copy'} - Optimized #${index + 1}`)
                                  if (!runName) return
                                  
                                  const newCopyTrade: CopyTrade = {
                                    id: Date.now().toString(),
                                    name: runName,
                                    traderAddress: group.runs[0].traderAddress,
                                    initialBudget: mcBudget,
                                    currentBudget: mcBudget,
                                    fixedBetAmount: strategy.bettingMode === 'fixed' ? (strategy.fixedBet || 1) : 1,
                                    minTriggerAmount: strategy.minTrigger,
                                  ...
                                  if (railwayUrl) {
                                    try {
                                      ...
                                      setNotification({ type: 'success', message: `✅ Run "${runName}" created and synced to bot!` })
                                    } catch (error) {
                                      setNotification({ type: 'success', message: `✅ Run "${runName}" created locally!` })
                                    }
                                  } else {
                                    setNotification({ type: 'success', message: `✅ Run "${runName}" created!` })
                                  }
                                  setTimeout(() => setNotification(null), 3000)
                                }}
                                className="px-4 py-2 bg-indigo-500/20 hover:bg-indigo-500/30 text-indigo-400 font-medium text-sm rounded-lg transition-all whitespace-nowrap"
                              >
                                🚀 Create Run
                              </button>
                            </div>
                          ...
                          <div className="grid grid-cols-4 gap-3 text-sm mb-3 pb-3 border-b border-slate-700/50">
                            <div>
                              <p className="text-xs text-slate-500 mb-1">🛡️ Bankruptcy Risk</p>
                              <p className={`font-bold text-lg ${strategy.bankruptcyRate === 0 ? 'text-emerald-400' : strategy.bankruptcyRate < 3 ? 'text-green-400' : 'text-orange-400'}`}>
                                {strategy.bankruptcyRate.toFixed(1)}%
                              </p>
                            </div>
                            <div>
                              <p className="text-xs text-slate-500 mb-1">📊 Consistency</p>
                              <p className="font-bold text-lg text-cyan-400">{strategy.consistency.toFixed(0)}%</p>
                            </div>
                            <div>
                              <p className="text-xs text-slate-500 mb-1">💹 ROI/Trade</p>
                              <p className="font-bold text-lg text-purple-400">
                                {strategy.avgRoiPerTrade ? strategy.avgRoiPerTrade.toFixed(2) : '0.00'}%
                              </p>
                            </div>
                            <div>
                              <p className="text-xs text-slate-500 mb-1">📉 Max Drawdown</p>
                              <p className="font-bold text-lg text-orange-400">{strategy.maxDrawdown.toFixed(1)}%</p>
                            </div>
                          </div>
                          
                          <div className="grid grid-cols-3 gap-3 text-sm mb-3 pb-3 border-b border-slate-700/50">
                            <div>
                              <p className="text-xs text-slate-500 mb-1">🎯 Trades to +10%</p>
                              <p className="font-semibold text-emerald-400">
                                {strategy.avgTradesToProfit10 && strategy.avgTradesToProfit10 > 0 ? Math.round(strategy.avgTradesToProfit10) : 'N/A'}
                              </p>
                            </div>
                            <div>
                              <p className="text-xs text-slate-500 mb-1">🎯 Trades to +25%</p>
                              <p className="font-semibold text-green-400">
                                {strategy.avgTradesToProfit25 && strategy.avgTradesToProfit25 > 0 ? Math.round(strategy.avgTradesToProfit25) : 'N/A'}
                              </p>
                            </div>
                            <div>
                              <p className="text-xs text-slate-500 mb-1">🎯 Trades to +50%</p>
                              <p className="font-semibold text-cyan-400">
                                {strategy.avgTradesToProfit50 && strategy.avgTradesToProfit50 > 0 ? Math.round(strategy.avgTradesToProfit50) : 'N/A'}
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-5 gap-3 text-xs text-slate-400">
                            <div>
                              <span className="text-slate-500">Survival:</span> <span className="text-emerald-400 font-medium">{strategy.survivalRate.toFixed(0)}%</span>
                            </div>
                            <div>
                              <span className="text-slate-500">Win Rate:</span> <span className="text-cyan-400 font-medium">{strategy.avgWinRate.toFixed(1)}%</span>
                            </div>
                            <div>
                              <span className="text-slate-500">Avg P&L:</span> <span className={`font-medium ${strategy.avgPnl >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>${strategy.avgPnl.toFixed(2)}</span>
                            </div>
                            <div>
                              <span className="text-slate-500">Sharpe:</span> <span className="text-purple-400 font-medium">{strategy.sharpeRatio.toFixed(2)}</span>
                            </div>
                            <div>
                              <span className="text-slate-500">Worst Streak:</span> <span className="text-red-400 font-medium">{strategy.worstStreak || 0} losses</span>
                            </div>
                          </div>
                        </div>
                      ))}
